...
{//TODO: Refactor using the accurate terminology:
    ...
    static void Main(string[] args)
    {
        ...
        List<AdjustedGravimetricPoint> adjustedPointValues = adjustment.CalculateUnknownPoints(adjustedMeas.Cast<RelativeMeasurement>().ToList());
        //AdjustmentResult<RelativeMeasurement, AdjustedGravimetricPoint> result = adjustment.AdjustNetwork();
        //Console.WriteLine("---");
        ...
        foreach (AdjustedGravimetricPoint kp in adjustedPointValues)
        {
            Console.WriteLine($"Изравнената точка {kp.Number} има потенциал {kp.GravitationalPotential}");
        }
